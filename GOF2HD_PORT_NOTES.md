...
скорость ∝ отклонению (deadzone 4000, макс 6.4 px/кадр ≈ 192 px/с),
...
**Урок эксперимента:** возвращать движку «живые» FMOD-объекты нельзя —
виртуальные вызовы должны стыковаться с реальной раскладкой FMOD-заголовков,
которой тогда не было.

### 11.4. Результат эксперимента

После перехода на реальные `libfmodex.so` и `libfmodevent.so` из APK эти
заглушки и связанные с ними патчи движка были удалены. Финальная конфигурация
не подменяет объекты FMOD: события, голоса и окончание реплик обрабатывает
оригинальная библиотека. Актуальная схема запуска описана в §12.5.4.
...
4. **`FMOD_Memory_Initialize`** временно использовался как диагностический
   обход раннего падения. В финальной конфигурации он не вызывается: запуск
   без override проверен на консоли, музыка и реплики работают.
...
### 12.5.4. Финальная конфигурация (2026-08-11)
...
Pacing OpenSL проверяет очередь SDL в середине PCM-блока, а не только после
его номинального завершения. Прежний интервал позволял `SDL_QueueAudio`
регулярно опускаться до нуля, что давало треск примерно раз в две секунды.
Раннее освобождение следующего FMOD-буфера сохраняет запас в несколько блоков
без изменения PCM или частоты воспроизведения.

`FMOD_Memory_Initialize` с пользовательскими malloc callbacks в финальном
runtime не вызывается: проверочный запуск без override подтвердил звук и
реплики.
